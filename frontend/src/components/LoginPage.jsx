...
const callback =  serverUrl + ':' + port + '/callback';
    ...
    <div className='flex flex-col items-center justify-between h-screen w-screen py-48'>
      ...
      <div>
        <label className="inline-flex items-center">
          <input type="checkbox" className="w-4 h-4 border-0 focus:ring-0" checked={checked} onClick={() => setChecked(!checked)} />
          <span className="ml-2">I accept the terms and agreements.</span>
        </label>
      </div>
      <div className="flex items-center justify-center w-fit mb-5 space-x-7">
        <button className={checked ? buttonStyleActive : buttonStyleDisabled} disabled={!checked} onClick={() => setClicked(true)} >
